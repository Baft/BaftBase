...
class condition implements sql\Predicate\PredicateInterface, \reportgen\flattenTree\nodeInterface {
	 ...
	 * @param array $vars
	 *        	variables in order to replace by '?' in condition string
	 * @return \reportgen\reportDefenition\condition
	 */
	public static function &create($name, $predicate, $combination = "AND", array $vars = array()) {

		$condition = new static ( $name );
		$condition->addCondition ( $predicate, $combination, $vars );
		return $condition;
	 ...
	 * @return \reportgen\reportDefenition\condition
	 */
	public function addCondition($predicates, $combination = "AND", array $vars = array()) {
