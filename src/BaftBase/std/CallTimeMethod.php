 ...
 *
 * //add method to class
 * //add a beforeCall method (third parameter) to call before each time call method "get"
 * $test->addMethod(
 * 'get',
 * function($propertyName){ return $this->{$propertyName};} ,
 * function(&$parameters){ if($parameters[0]=='family') $parameters[0]='name'; }
 * );
 *
 * //make previously added method "get" responsible to all method calls with Name like "get[customeString]" eg.: getName()
 * $test->addMethodPrefix(
 * 'get',
 * 'get'
 * );
 *
 * //make "find" method to responsible to all method calls with Name like "find[customeString]" eg.: findUser()
 * $test->addMethodPrefix(
 * 'find',
 * 'find'
 * );
 *
 * //use
 * $test->get('name'); // call own method . return $name property value = 'casra'
 *
 * $test->getName(); //call by prefix .change by beforeCall callback (third parameter of addMethodPrefix). return $name property value = 'casra'
 *
 * $test->getFamily(); //call by prefix .casra
 *
 * $test->findUser('abc');
 */
trait CallTimeMethod {
